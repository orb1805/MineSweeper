...
class AutoPlayActivity : AppCompatActivity(), Drawer, PlayGround {
...
//switchBtn = findViewById(R.id.switcher_btn)
        ...
        layout.removeView(numberLayout)
        ...
        drawBtn.setOnClickListener {
            if (rowsET.text.isNotEmpty() && columnsET.text.isNotEmpty() && bombsET.text.isNotEmpty()) {
                ...
                draw(size)
                player = Player(rows, columns, bombs, this)
                //player.play()
                ...
                MotionEvent.ACTION_UP -> {
                    if (x == xDown && y == yDown) {
                        ...
                        if (i in 0 until rows && j in 0 until columns/* && playing == 0*/) {
                            ...
                            markX = i
                            markY = j
                            text.text = "$markX $markY ${field.size}"
                            /*if (firstTap) {
                                firstTap = false
                                regenerate(i, j)
                                open(i, j)
            ...
            for (j in field[i].indices) {
                if (field[i][j] != -1) {
                    if (field[i][j] == 100){
                        ...
                        canvas.drawText(
                                field[i][j].toString(),
                                (j * dCols + dCols / 3 + transCols) * this.size,
                                ((i + 1) * dRows - dRows / 3 + transRows) * this.size,
                                paintBlack
                        )
                    }
                }
    ...
    override fun reveal(i: Int, j: Int) {
        finished = false
        var dRows = height / rows
        var dCols = width / columns
        var transCols = 0f
        ...
        if (i in 0 until rows && j in 0 until columns/* && playing == 0*/) {
            ...
            canvas.drawLine(
                (j * dCols + transCols) * this.size,
                (i * dRows + transRows) * this.size,
                ((j + 1) * dCols + transCols) * this.size,
                ((i + 1) * dRows + transRows) * this.size,
                paintBlack
            )
            canvas.drawLine(
                (j * dCols + transCols) * this.size, (dRows * (i + 1) + transRows) * this.size,
                ((j + 1) * dCols + transCols) * this.size, (i * dRows + transRows) * this.size,
                paintBlack
            )
            paintBlack.color = Color.BLACK
    ...
    override fun shareField(): MutableList<MutableList<Int>> {
        return field
    }

    override fun massage(msg: String) {
